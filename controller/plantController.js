...
module.exports={
    ...
    DeleteById:function(req, res){
        ...
        Plant.deleteOne({id : req.params.id}, function(err, result){
            if (err) throw err;
            //Renders the list of all Plants in the path /
            res.redirect('/');
        });
        console.log("The row which is being deleted: %s", {id : req.params.id})
    },
    //Get function ID to Update plants
    GetupdateId:function(req, res){
        console.log('Update the row using id');
        Plant.findOne({id : req.params.id}, function(err, results){
            if(err) throw err;
            //Renders the list of all Plants in the updateplant.ejs file
            res.render('updateplant.ejs',{theplant:results});
        });
        console.log("The id is %s", {id : req.params.id})
    },
